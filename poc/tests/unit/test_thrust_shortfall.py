...
def test_thrust_altitude_loss():
    """Test thrust fault reduces altitude when active."""
    fault = ThrustShortfallFault(every_sec=0.0, duration_sec=10.0, alt_loss_mps=2.0)
    ...
    # Altitude should be reduced (minimum 0.01s worth of loss)
    assert result is not None
    assert result.position.alt_m < original_alt
    ...
    # First call had ~0.01s loss, second call ~0.1s total loss
    assert result2.position.alt_m < original_alt - 0.15  # At least 0.15s * 2m/s loss
...
def test_thrust_loss_rate():
    ...
    altitude_lost = original_alt - result2.position.alt_m
    
    # Loss accumulates from fault start, should be approximately alt_loss_mps * (sleep_time + initial)
    # Expect at least sleep_time worth of loss
    expected_min_loss = alt_loss_mps * sleep_time * 0.8  # 80% of expected (account for timing jitter)
    expected_max_loss = alt_loss_mps * (sleep_time + 0.1)  # Allow extra for timing
    
    assert altitude_lost >= expected_min_loss
    assert altitude_lost <= expected_max_loss
